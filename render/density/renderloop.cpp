...
static unsigned long long fpsCount;
static double timeBegin;

/***********************/

static void beginDeviceCoords(void)
{
...
static void glutStrokePrint(float x, float y, const char *s, void *font)
{
  glPushMatrix();
...
static void glPrintf(float x, float y, const char* format, ...)                                
{                                                                                       
...
static void drawWireBox(float3 boxMin, float3 boxMax) 
{
  glLineWidth(1.0);
...
{  
  ...
  glutCreateWindow("Bonsai N-body Visualization");
  //if (bFullscreen)
  //  glutFullScreen();
  GLenum err = glewInit();
